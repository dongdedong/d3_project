/*! vizuly 28-04-2016 */
function loadData() {
    d3.csv("data/halo_contributions_house.csv", function (a) {
...
function initialize() {
    viz = vizuly.viz.halo_cluster(document.getElementById("viz_container")), viz.data(data).width(800).height(600).haloKey(function (a) {
        ...
        return Number(a.TRANSACTION_AMT)
    }).on("update", onUpdate).on("nodeover", node_onMouseOver).on("nodeout", onMouseOut).on("arcover", arc_onMouseOver).on("arcout", onMouseOut).on("linkover", link_onMouseOver).on("linkout", onMouseOut), theme = vizuly.theme.halo(viz), theme.skins().custom = customSkin, theme.skin("custom"), viz_title = viz.selection().select("svg").append("text").attr("class", "title").style("font-family", "Raleway").attr("x", viz.width() / 2).attr("y", 40).attr("text-anchor", "middle").style("font-weight", 300).text("POLITICAL INFLUENCE"), changeSize(d3.select("#currentDisplay").attr("item_value"))
}
function onUpdate() {
    viz_title.style("fill", theme.skin() != customSkin ? theme.skin().labelColor : "#000")
}
function node_onMouseOver(a, b, c) {
    var d = {}, e = viz.selection().selectAll(".vz-halo-link-path.node-key_" + b.key), f = 0;
    e.each(function (a) {
        f += viz.value()(a.data);
        var b = viz.selection().selectAll(".vz-halo-arc.halo-key_" + viz.haloKey()(a.data));
        b.each(function (a) {
            d[a.data.key] || (d[a.data.key] = 1, createPacLabel(a.x, a.y, a.data.values[0].CMTE_NM))
        })
    }), f = "$" + d3.format(",.2f")(f);
    var g = d3.selectAll(".vz-halo-arc-plot")[0][0].getBoundingClientRect(), h = viz.selection().selectAll(".vz-halo-node.node-key_" + b.key), g = h[0][0].getBoundingClientRect(), i = g.left, j = g.top + document.body.scrollTop;
    createDataTip(i + b.r, j + b.r + 25, congress, b.values[0].CAND_NAME, "Total Received: " + f)
}
function arc_onMouseOver(a, b, c) {
    var d = viz.selection().selectAll(".vz-halo-link-path.halo-key_" + b.data.key), e = 0;
    d.each(function (a) {
        e += viz.value()(a.data)
    }), e = "$" + d3.format(",.0f")(e);
    var f = d3.selectAll(".vz-halo-node-plot")[0][0].getBoundingClientRect();
    createDataTip(b.x + f.left + f.width / 2, b.y + f.top + 100, "POLITICAL ACTION COMMITTEE", b.data.values[0].CMTE_NM, "Total Donated: " + e)
}
function link_onMouseOver(a, b, c) {
    var d = viz.selection().selectAll(".vz-halo-node.node-key_" + viz.nodeKey()(b.data)), e = d.datum(), f = "$" + d3.format(",.0f")(viz.value()(b.data)), g = b.data.Month + "/" + b.data.Day + "/" + b.data.Year, h = d3.selectAll(".vz-halo-arc-plot")[0][0].getBoundingClientRect();
    createDataTip(e.x + h.left + e.r, e.y + e.r + 25 + h.top, g, b.data.CAND_NAME, "Received: " + f);
    var i = viz.selection().selectAll(".vz-halo-arc.halo-key_" + viz.haloKey()(b.data));
    e = i.datum(), createPacLabel(e.x, e.y, e.data.values[0].CMTE_NM)
}
function createDataTip(a, b, c, d, e) {
    var f = datatip.replace("HEADER1", c);
    f = f.replace("HEADER2", d), f = f.replace("HEADER3", e), d3.select("body").append("div").attr("class", "vz-halo-label").style("position", "absolute").style("top", b + "px").style("left", a - 125 + "px").style("opacity", 0).html(f).transition().style("opacity", 1)
}
function createPacLabel(a, b, c) {
    ...
    var e = d[0][0].getBoundingClientRect();
    d.insert("rect", "text").style("shape-rendering", "auto").style("fill", theme.skin().labelFill).style("opacity", .45).attr("width", e.width + 12).attr("height", e.height + 12).attr("rx", 3).attr("ry", 3).attr("x", a - 5 - e.width / 2).attr("y", b - e.height - 3), d.transition().style("opacity", 1)
}
...
function changeSize(a) {
    ...
    viz_container.transition().duration(300).style("width", b[0] + "px").style("height", b[1] + "px"), viz.width(Number(b[0])).height(Number(b[1])).update(), viz_title.attr("x", viz.width() / 2), theme.apply()
}
...
vizuly.version = "1.0", vizuly.ui = {}, vizuly.viz = {}, vizuly.viz.layout = {}, vizuly.viz.layout.CLUSTERED = "CLUSTERED", vizuly.viz.layout.STACKED = "STACKED", vizuly.viz.layout.OVERLAP = "OVERLAP", vizuly.viz.layout.STREAM = "STREAM", vizuly.color = {}, vizuly.color.shift = function (a, b) {
    ...
    function e(a, f) {
        for (var g = 0; g < a.length; g++) {
            ...
            if (h.values) {
                e(h.values, h);
                for (var i = 0; i < h.values.length; i++)for (var j = h.values[i], k = 0; k < b.length; k++)isNaN(h["agg_" + b[k]]) && (h["agg_" + b[k]] = 0), h["agg_" + b[k]] = c(h["agg_" + b[k]], j["agg_" + b[k]])
            } else for (var k = 0; k < b.length; k++)h["agg_" + b[k]] = Number(h[b[k]]), isNaN(h["agg_" + b[k]]) && (h["agg_" + b[k]] = 0);
            d(h, f)
        }
    }
    ...
    var e = Math.round(100 * b) + "_" + Math.round(100 * c) + "_" + Math.round(100 * d), f = a.id(), g = vizuly.util.getDefs(a), h = g.selectAll("#vz_filter_" + f + "_" + e).data([e]).enter().append("filter").attr("id", "vz_filter_" + f + "_" + e).attr("class", "vz-svg-filter-dropShadow").attr("width", "300%").attr("height", "300%");
    ...
    var e, f, g, h, i = String(b).replace("#", "") + String(c).replace("#", ""), j = "vz_gradient_blend_" + a.id() + "_" + i;
    "horizontal" == d ? (e = "100%", f = "0%", g = "0%", h = "0%") : (e = "0%", f = "0%", g = "100%", h = "0%");
    var k = vizuly.util.getDefs(a), l = k.selectAll("#" + j).data([i]).enter().append("linearGradient").attr("id", j).attr("class", "vz-svg-gradient-blend").attr("x1", e).attr("x2", f).attr("y1", g).attr("y2", h);
    ...
    e || (e = [0, 1]), d || (d = [.75, .9]);
    var f, g, h, i, j = String(b).replace("#", ""), k = "vz_gradient_fade_" + a.id() + "_" + j;
    "horizontal" == c ? (f = "0%", g = "100%", h = "0%", i = "0%") : (f = "0%", g = "0%", h = "100%", i = "0%");
    var l = vizuly.util.getDefs(a), m = l.selectAll("#" + k).data([j]).enter().append("linearGradient").attr("id", k).attr("class", "vz-svg-gradient-fade").attr("x1", f).attr("x2", g).attr("y1", h).attr("y2", i);
    ...
    d || (d = [0, 1]), c || (c = [.75, .9]);
    var e = String(b).replace("#", ""), f = "vz_gradient_radial_fade" + a.id() + "_" + e, g = vizuly.util.getDefs(a), h = g.selectAll("#" + f).data([e]).enter().append("radialGradient").attr("id", f).attr("class", "vz-svg-gradient-radial-fade");
    ...
    var d, e, f, g, h = String(b).replace("#", ""), i = "vz_gradient_darker_" + a.id() + "_" + h;
    "horizontal" == c ? (d = "100%", e = "0%", f = "0%", g = "0%") : (d = "0%", e = "0%", f = "100%", g = "0%");
    var j = vizuly.util.getDefs(a), k = j.selectAll("#" + i).data([h]).enter().append("linearGradient").attr("class", "vz-gradient-darker").attr("id", i).attr("x1", d).attr("x2", e).attr("y1", f).attr("y2", g);
    ...
    var e = d3.svg.arc().innerRadius(a).outerRadius(a)(d), f = /[Mm][\d\.\-e,\s]+[Aa][\d\.\-e,\s]+/, g = f.exec(e);
    return g && (g = g[0]), g
}, vizuly.svg.text.wrap = function (a, b, c) {
    c = !c && a.attr("width") ? Number(a.attr("width")) : c, a.each(function () {
        var a = d3.select(this);
        a.selectAll("tspan").remove();
        var d, e = a.text().split(/\s+/).reverse(), f = [], g = 0, h = a.attr("x"), i = a.attr("y"), j = parseFloat(b);
        isNaN(j) && (j = 0);
    ...
    }, a = a, o = [{on: "measure.theme", callback: f}, {on: "update.theme", callback: c}, {
        on: "mouseover.theme",
        callback: d
    }, {on: "mouseout.theme", callback: e}];
    b();
    ...
    var a = a, i = null, j = vizuly.svg.gradient.blend(a, "#000", "#000"), k = d3.scale.category20(), l = [{
        on: "measure.theme",
        callback: f
    }, {on: "update.theme", callback: c}, {on: "mouseover.theme", callback: d}, {on: "mouseout.theme", callback: e}];
    ...
    var a = a, s = null, t = vizuly.svg.gradient.blend(a, "#000", "#000"), u = d3.scale.linear(), v = [{
        on: "measure.theme",
        callback: p
    }, {on: "update.theme", callback: c}, {on: "nodeover.theme", callback: f}, {
        on: "nodeout.theme",
        callback: g
    }, {on: "arcover.theme", callback: d}, {on: "arcout.theme", callback: g}, {
        on: "linkover.theme",
        callback: e
    }, {on: "linkout.theme", callback: g}];
    b(), b.apply = function (a) {
        return arguments.length > 0 && b.skin(a), c(), b
    }, b.release = function () {
    ...
    var w = {
        ...
        Ocean: {
            name: "Ocean", labelColor: "#FFF", labelFill: "#000", background_transition: function (b) {
                a.selection().select(".vz-background").transition(1e3).style("fill-opacity", 0)
            }, link_stroke: function (a, b) {
                return "#FFF"
            }, link_fill: function (a, b) {
                return "#FFF"
            }, link_fill_opacity: .075, link_node_fill_opacity: .075, node_stroke: function (a, b) {
                return "#FFF"
            }, node_over_stroke: function (a, b) {
                return "#FFF"
            }, node_fill: function (a, b) {
    ...
    var h = {
        ...
        White: {
            name: "White", label_color: "#FFF", track_fill: null, arc_fill: function (a, b) {
                return "#FFF"
            }, arc_fill_opacity: function (a, b) {
    ...
    }, a = a, i = [{on: "update.theme", callback: c}, {on: "mouseover.theme", callback: d}, {
        on: "mouseout.theme",
        callback: e
    }];
    b(), b.apply = function (a) {
    ...
    function c() {
        if (i) {
            var b = a.width(), c = Math.min(a.width(), a.height()) / 80, d = a.selection();
            d.attr("class", i["class"]), d.selectAll(".vz-background").attr("fill", function () {
                return "url(#" + j.attr("id") + ")"
    ...
    var a = a, i = null, j = vizuly.svg.gradient.blend(a, "#000", "#000"), k = [{
        on: "measure.theme",
        callback: d
    }, {on: "update.theme", callback: c}, {on: "mouseover.theme", callback: e}, {on: "mouseout.theme", callback: f}];
    b(), b.apply = function (a) {
        return arguments.length > 0 && b.skin(a), c(), b
    }, b.release = function () {
    ...
    var m = {
        ...
        Ocean: {
            name: "Ocean", labelColor: "#FFF", labelFill: "#000", background_transition: function (b) {
                a.selection().select(".vz-background").transition(1e3).style("fill-opacity", 0)
            }, yAxis_line_stroke: "#FFF", yAxis_line_opacity: .25, node_stroke: function (a, b) {
                return "#00F"
            }, node_fill: function (a, b) {
                return "#FFF"
            }, node_fill_opacity: function (a, b) {
    ...
    }, a = a, l = !0, m = !0, n = [{on: "update.theme", callback: c}, {
        on: "data_prepped.theme",
        callback: d
    }, {on: "mouseover.theme", callback: f}, {on: "mouseout.theme", callback: g}];
    b(), b.apply = function (a) {
        return arguments.length > 0 && b.skin(a), c(), b
    }, b.release = function () {
    ...
    return b
}, vizuly.skin.WEIGHTED_TREE_AXIIS = "Axiis", vizuly.viz.halo_cluster = function (a) {
    function b() {
        u = h.selection.append("svg").attr("id", h.id).style("overflow", "visible").attr("class", "vizuly"), C = vizuly.util.getDefs(k), w = u.append("rect").attr("class", "vz-background"), v = u.append("g").attr("class", "vz-halo-viz"), x = v.append("g").attr("class", "vz-halo-plot").attr("clip-path", "url(#" + h.id + "_plotClipPath)"), y = x.append("rect").attr("class", "vz-plot-background"), B = x.append("g").attr("class", "vz-halo-link-plot"), A = x.append("g").attr("class", "vz-halo-node-plot"), z = x.append("g").attr("class", "vz-halo-arc-plot"), h.dispatch.initialize()
    }

    function c() {
        k.validate(), 1 == l && (f(), B.selectAll("path").remove(), B.selectAll("circle").remove(), z.selectAll("path").remove(), A.selectAll("circle").remove(), l = !1), m = vizuly.util.size(h.margin, h.width, h.height), o = Math.min(m.width, m.height) / 2, n = o * (1 - h.haloThickness), H.size([2 * n * .99, 2 * n * .99]).padding(2).sort(null).value(function (a) {
            return a.value
        }).children(function (a) {
            return a.children
        }), F.padAngle(h.padAngle * D).startAngle(0).endAngle(360 * D).value(function (a) {
            return a.value
        }), G.value(h.value).padAngle(0), E.outerRadius(o).innerRadius(n), t = H.nodes({children: s}).filter(function (a) {
            return a.depth > 1
        }), h.dispatch.measure();
        for (var a in r)r[a].aggregate = 0
    }

    function d() {
        c(), u.attr("width", h.width).attr("height", h.height), w.attr("width", h.width).attr("height", h.height), x.style("width", m.width).style("height", m.height).attr("transform", "translate(" + m.left + "," + m.top + ")"), z.attr("transform", "translate(" + m.width / 2 + "," + m.height / 2 + ")"), B.attr("transform", "translate(" + m.width / 2 + "," + m.height / 2 + ")"), A.attr("transform", "translate(" + (m.width - 2 * n) / 2 + "," + (m.height - 2 * n) / 2 + ")");
        var a = z.selectAll(".vz-halo-arc").data(F(p));
        a.enter().append("path").attr("class", function (a) {
            return "vz-halo-arc halo-key_" + a.data.key
        }).on("mouseover", function (a, b) {
            h.dispatch.arcover(this, a, b)
        }).on("mouseout", function (a, b) {
            h.dispatch.arcout(this, a, b)
        }), a.exit().remove(), a.attr("d", function (a, b) {
            return E(a, b)
        }), a.each(function (a) {
            var b = a.startAngle + (a.endAngle - a.startAngle) / 2 - 90 * D;
            a.x = n * Math.cos(b), a.y = n * Math.sin(b)
        });
        var b = B.selectAll(".vz-halo-group").data(F(p));
        b.enter().append("g").attr("class", "vz-halo-group"), b.exit().remove(), b.each(function (a) {
            G.startAngle(a.startAngle + F.padAngle() / 1.415).endAngle(a.endAngle - F.padAngle() / 1.415);
            var b = d3.select(this).selectAll(".vz-halo-link").data(G(a.data.values));
            b.enter().append("g").attr("class", "vz-halo-link"), b.exit().remove(), b.selectAll("path").remove(), b.selectAll("circle").remove(), b.append("path").attr("class", function (a) {
                return "vz-halo-arc-slice node-key_" + h.nodeKey(a.data)
            }).attr("d", E);
            b.each(function (a) {
                var b = e(a), c = r[h.nodeKey(a.data)], d = d3.select(this);
                d.append("path").attr("class", "vz-halo-link-path node-key_" + h.nodeKey(a.data) + " halo-key_" + h.haloKey(a.data)).on("mouseover", function (a, b) {
                    h.dispatch.linkover(this, a, b)
                }).on("mouseout", function (a, b) {
                    h.dispatch.linkout(this, a, b)
                }).attr("d", function (a, c) {
                    var d = I(b.arc_to_node, c);
                    return d += "L" + String(I(b.node_to_arc, c)).substr(1), d += "A" + n + "," + n + " 0 0,0 " + b.arc_to_node.source.x + "," + b.arc_to_node.source.y
                }), c.aggregate = c.aggregate + h.value(a.data);
                var f = c.aggregate / c.value * c.r;
                d.append("circle").attr("class", "vz-halo-link-node node-key_" + h.nodeKey(a.data) + " halo-key_" + h.haloKey(a.data)).attr("r", 0).attr("cx", c.x - n).attr("cy", c.y - n).transition().duration(h.duration).attr("r", f)
            })
        });
        var d = A.selectAll(".vz-halo-node").data(t);
        d.enter().append("circle").attr("class", function (a) {
            return "vz-halo-node node-key_" + a.key
        }).on("mouseover", function (a, b) {
            h.dispatch.nodeover(this, a, b)
        }).on("mouseout", function (a, b) {
            h.dispatch.nodeout(this, a, b)
        }), d.exit().remove(), d.attr("r", 0).attr("cx", function (a) {
            return a.x
        }).attr("cy", function (a) {
            return a.y
        }).transition().duration(h.duration).attr("r", function (a) {
            return a.r
        }), h.dispatch.update()
    }

    function e(a) {
        var b = r[h.nodeKey(a.data)], c = {};
        c.data = a.data;
        var d = {};
        d.source = {}, d.source.x = n * Math.cos(a.startAngle - 1.57079633), d.source.y = n * Math.sin(a.startAngle - 1.57079633), d.target = {}, d.target.x = b.x - n, d.target.y = b.y - n;
        var e = {};
        return e.target = {}, e.target.x = n * Math.cos(a.endAngle - 1.57079633), e.target.y = n * Math.sin(a.endAngle - 1.57079633), e.source = d.target, c.arc_to_node = d, c.node_to_arc = e, c
    }

    function f() {
        p = d3.nest().key(h.haloKey).entries(h.data), q = d3.nest().key(h.nodeKey).entries(h.data);
        var a = d3.nest().key(h.nodeGroupKey).key(h.nodeKey).entries(h.data);
        g(p), g(q), r = [], q.forEach(function (a) {
            a.nodeGroup = h.nodeGroupKey(a.values[0]), r[h.nodeKey(a.values[0])] = a
        }), s = [], a.forEach(function (a) {
            var b = {};
            b.id = a.key, b.values = q.filter(function (b) {
                return a.key == b.nodeGroup
            }), b.children = b.values, s.push(b)
        }), g(s, "value")
    }
    ...
    function g(a, b) {
        a.forEach(function (a) {
            ...
            a.values.forEach(function (a) {
                c += b ? Number(a[b]) : Number(h.value(a))
            }), a.value = c
        })
    }
    ...
    var h = {}, i = {
        data: null,
        margin: {top: "10%", bottom: "7%", left: "9%", right: "7%"},
        duration: 500,
        width: 300,
        height: 300,
        padAngle: 1,
        ...
        nodeGroupKey: null,
        value: null
    }, j = ["linkover", "nodeover", "arcover", "linkout", "nodeout", "arcout"], k = vizuly.component.create(a, h, i, j);
    k.type = "viz.chart.halo_cluster";
    var l = !1;
    k.on("data_change.internal", function () {
        l = !0
    });
    var m, n, o, p, q, r, s, t, u, v, w, x, y, z, A, B, C, D = Math.PI / 180, E = d3.svg.arc(), F = d3.layout.pie(), G = d3.layout.pie(), H = d3.layout.pack(), I = d3.svg.diagonal.radial();
    return b(), k.update = function () {
        return d(), k
    }, k
};
var viz, viz_container, viz_title, theme, data, dataSource = {}, formatDate = d3.time.format("%b %d, 20%y"), datatip = '<div class="tooltip" style="width: 250px; background-opacity:.5"><div class="header1">HEADER1</div><div class="header-rule"></div><div class="header2"> HEADER2 </div><div class="header-rule"></div><div class="header3"> HEADER3 </div></div>', repColor = "#F80018", demColor = "#0543bc", otherColor = "#FFa400", customSkin = {
    name: "custom",
    labelColor: "#FFF",
    labelFill: "#000",
    background_transition: function (a) {
        viz.selection().select(".vz-background").transition(1e3).style("fill-opacity", 0)
    },
    link_stroke: function (a, b) {
        return "REP" == a.data.PTY ? repColor : (a.nodeGroup = "DEM") ? demColor : otherColor
    },
    link_fill: function (a, b) {
        return "REP" == a.data.PTY ? repColor : (a.nodeGroup = "DEM") ? demColor : otherColor
    },
    ...
    node_fill: function (a, b) {
        return "REP" == a.nodeGroup ? repColor : "DEM" == a.nodeGroup ? demColor : otherColor
    },
    arc_stroke: function (a, b) {
        return "#FFF"
    },
    arc_fill: function (a, b) {
        return "REP" == a.data.PTY ? repColor : "DEM" == a.nodeGroup ? demColor : otherColor
    },
    arc_over_fill: function (a, b) {
        return "#000"
    },
