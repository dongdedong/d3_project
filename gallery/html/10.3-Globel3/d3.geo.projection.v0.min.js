!function () {
    ...
    function y(t) {
        function n(t, n) {
            var r = Ta(t, n);
            if (Math.abs(t) > wa) {
                var e = Math.atan2(r[1], r[0]), o = Math.sqrt(r[0] * r[0] + r[1] * r[1]), i = a * Math.round((e - wa) / a) + wa, u = Math.atan2(Math.sin(e -= i), 2 - Math.cos(e));
        ...
        return n.invert = function (t, n) {
            ...
            if (r > wa) {
                var e = Math.atan2(n, t), o = a * Math.round((e - wa) / a) + wa, i = e > o ? -1 : 1, h = r * Math.cos(o - e), u = 1 / Math.tan(i * Math.acos((h - pa) / Math.sqrt(pa * (pa - 2 * h) + r * r)));
    ...
    function Y(t, n) {
        function a(n, a) {
            ...
            if (u > e) {
                var M = Math.sqrt(u), s = Math.atan2(h, i), c = r * Math.round(s / r), f = s - c, v = t * Math.cos(f), l = (t * Math.sin(f) - f * Math.sin(v)) / (wa - v), g = Z(f, l), d = (pa - t) / tn(g, v, pa);
                i = M;
                var b, p = 50;
        ...
        return a.invert = function (n, a) {
            ...
            if (o > e) {
                ...
                for (var s = n - wa, c = Math.sin(n), f = a / c, v = wa > n ? 1 / 0 : 0, l = 10; ;) {
                    var g = t * Math.sin(f), d = t * Math.cos(f), b = Math.sin(d), p = wa - d, w = (g - f * b) / p, q = Z(f, w);
    ...
    function $() {
        var t = 6, n = 30 * ma, a = Math.cos(n), r = Math.sin(n), e = Qa(Y), o = e(n, t), i = o.stream, h = .01, u = -Math.cos(h * ma), M = Math.sin(h * ma);
    ...
    function nn(t, n, a, r, e, o, i, h) {
        function u(u, M) {
            ...
            var s = M * M, c = t + s * (n + s * (a + s * r)), f = M * (e - 1 + s * (o - h + s * i)), v = (c * c + f * f) / (2 * f), l = u * Math.asin(c / v) / pa;
        ...
        return arguments.length < 8 && (h = 0), u.invert = function (u, s) {
            ...
            do {
                var d = l * l, b = t + d * (n + d * (a + d * r)), p = l * (e - 1 + d * (o - h + d * i)), w = b * b + p * p, q = 2 * p, m = w / q, y = m * m, S = Math.asin(b / m) / pa, Q = v * S;
    ...
    function rn(t) {
        ...
        function a(n) {
            var a = n ? function (n, a) {
                var e = Math.abs(n) < wa, o = t(e ? n : n > 0 ? n - pa : n + pa, a), h = (o[0] - o[1]) * Math.SQRT1_2, u = (o[0] + o[1]) * Math.SQRT1_2;
                if (e)return [h, u];
    ...
    function on(t, n) {
        ...
        var b = Math.cos(n), p = 1 / b, w = 2 * a * b, q = (-3 * r + s * i) * w, m = (-c * b - (1 - a) * q) / (c * c), y = .5 * m / v, S = u * y - 2 * r * v * w, Q = r * o * m + f * i * w, R = -p * w, T = -p * Q, x = -2 * p * S, E = 4 * t / pa;
    ...
    function fn(t, n) {
        var a = (Math.SQRT2 - 1) / (Math.SQRT2 + 1), r = Math.sqrt(1 - a * a), e = cn(wa, r * r), o = -1, i = Math.log(Math.tan(pa / 4 + Math.abs(n) / 2)), h = Math.exp(o * i) / Math.sqrt(a), u = vn(h * Math.cos(o * t), h * Math.sin(o * t)), M = sn(u[0], u[1], r * r);
    ...
    function gn(t) {
        function n(t, n) {
            var o = e(t, n);
            t = o[0], n = o[1];
            var i = Math.sin(n), h = Math.cos(n), M = Math.cos(t), s = u(a * i + r * h * M), c = Math.sin(s), f = Math.abs(c) > da ? s / c : 1;
        ...
        return e.invert = dn(-t), n.invert = function (t, n) {
            var r = Math.sqrt(t * t + n * n), o = -Math.sin(r), i = Math.cos(r), h = r * i, u = -n * o, s = r * a, c = M(h * h + u * u - s * s), f = Math.atan2(h * s + u * c, u * s - h * c), v = (r > wa ? -1 : 1) * Math.atan2(t * o, r * Math.cos(f) * i + n * Math.sin(f) * o);
            return e.invert(v, f)
        }, n
    }
    ...
    function yn(t) {
        ...
        var a = d3.geo.cylindricalEqualArea.raw(0), r = Ca * pa / 180, e = 2 * pa, o = d3.geo.collignon.raw(pa, r)[0] - d3.geo.collignon.raw(-pa, r)[0], i = a(0, r)[1], h = d3.geo.collignon.raw(0, r)[1], u = d3.geo.collignon.raw(0, wa)[1] - h, M = 2 * pa / t;
        return n.invert = function (n, r) {
            n *= 2;
            var s = Math.abs(r);
            if (s > i) {
    ...
    function Fn(t) {
        function n(n, e) {
            var o = e - t, i = Math.abs(o) < da ? n * a : Math.abs(i = pa / 4 + e / 2) < da || Math.abs(Math.abs(i) - wa) < da ? 0 : n * o / Math.log(Math.tan(i) / r);
            return [i, o]
        }
    ...
    function Kn(t) {
        ...
        return n.invert = function (n, r) {
            ...
            var v = n * (s = Math.tan(i)), l = Math.tan(Math.abs(r) < Math.abs(i + 1 / s) ? .5 * h(v) : .5 * u(v) + pa / 4) / Math.sin(i);
    ...
    function Zn() {
        ...
        return delete a.rotate, a.points = function (a) {
            if (!arguments.length)return t;
            t = a;
            var e = d3.geo.interpolate(a[0], a[1]), o = e(.5), i = d3.geo.rotation([-o[0], -o[1]])(a[0]), u = .5 * e.distance, M = -h(Math.sin(i[1] * ma) / Math.sin(u));
    ...
    function na(t, n) {
        ...
        var e = Math.cos(r), o = Math.abs(pa / t - t / pa) / 2, u = o * o, M = e / (a + e - 1), s = M * (2 / a - 1), c = s * s, f = c + u, v = M - c, l = u + M;
    ...
    function aa(t, n) {
        ...
        var a = Math.abs(n / wa), r = h(a);
        if (Math.abs(t) < da || Math.abs(Math.abs(n) - wa) < da)return [0, i(n) * pa * Math.tan(r / 2)];
        var e = Math.cos(r), o = Math.abs(pa / t - t / pa) / 2, u = o * o, s = e * (Math.sqrt(1 + u) - o * e) / (1 + u * a * a);
    ...
    function ea(t, n) {
        ...
        var r = a / wa, e = r * r, o = (8 * r - e * (e + 2) - 5) / (2 * e * (r - 1)), h = o * o, u = r * o, s = e + h + 2 * u, c = r + 3 * o, f = t / wa, v = f + 1 / f, l = i(Math.abs(t) - wa) * Math.sqrt(v * v - 4), g = l * l, d = s * (e + h * g - 1) + (1 - e) * (e * (c * c + 4 * h) + 12 * u * h + 4 * h * h), b = (l * (s + h - 1) + 2 * M(d)) / (4 * s + g);
    ...
    var Ma = {
        ...
        MultiPolygon: la,
        Sphere: la
    }, da = 1e-6, ba = da * da, pa = Math.PI, wa = pa / 2, qa = Math.sqrt(pa), ma = pa / 180, ya = 180 / pa, Sa = d3.geo.projection, Qa = d3.geo.projectionMutator;
    d3.geo.interrupt = function (t) {
        ...
        function r() {
            for (var t = 1e-6, n = [], a = 0, r = h[0].length; r > a; ++a) {
                var o = h[0][a], i = 180 * o[0][0] / pa, u = 180 * o[0][1] / pa, M = 180 * o[1][1] / pa, s = 180 * o[2][0] / pa, c = 180 * o[2][1] / pa;
                n.push(e([[i + t, u + t], [i + t, M - t], [s - t, M - t], [s - t, c + t]], 30))
            }
            for (var a = h[1].length - 1; a >= 0; --a) {
                var o = h[1][a], i = 180 * o[0][0] / pa, u = 180 * o[0][1] / pa, M = 180 * o[1][1] / pa, s = 180 * o[2][0] / pa, c = 180 * o[2][1] / pa;
    ...
    (d3.geo.ginzburg9 = function () {
        ...
        var a = (Math.SQRT2 - 1) / (Math.SQRT2 + 1), r = Math.sqrt(1 - a * a), e = cn(wa, r * r), o = -1, i = un(.5 * e - n, -t, r * r), h = ln(i[0], i[1]), u = Math.atan2(h[1], h[0]) / o;
    ...
    var Ha = 1.0148, Ia = .23185, Ja = -.14499, Ka = .02406, Na = Ha, Ua = 5 * Ia, Va = 7 * Ja, Wa = 9 * Ka, Xa = 1.790857183;
    ...
    Za.forEach(function (t) {
        ...
        do {
            var i = Za[o][1], h = Za[o + 1][1], u = Za[Math.min(19, o + 2)][1], M = u - i, s = u - 2 * h + i, c = 2 * (Math.abs(a) - h) / M, f = s / M, v = c * (1 - f * c * (1 - 2 * f * c));
            if (v >= 0 || 1 === o) {
                r = (n >= 0 ? 5 : -5) * (v + e);
                var l, g = 50;
        ...
        var a = (t /= pa) * t, r = (n /= pa) * n, e = a + r, o = e * e, h = -Math.abs(n) * (1 + e), M = h - 2 * r + a, s = -2 * h + 1 + 2 * r + o, c = r / s + (2 * M * M * M / (s * s * s) - 9 * h * M / (s * s)) / 27, f = (h - M * M / (3 * s)) / s, v = 2 * Math.sqrt(-f / 3), l = u(3 * c / (f * v)) / 3;
