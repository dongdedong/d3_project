function Flight(a, b) {
    this.map = a, this.svg = b, this.curZoom = this.map.getZoom(), this.beginPoint = {
        ...
        var c = Math.random() < .5 ? -1 : 1, d = (a.lat + b.lat) / 2, e = (a.lng + b.lng) / 2;
        this.midPoint.lat = (a.lat + b.lat) / 2 + Math.random() * d * .25 * c, this.midPoint.lng = (a.lng + b.lng) / 2 + Math.random() * e * .25 * c, this.mp_px = this.map.latLngToLayerPoint([this.midPoint.lat, this.midPoint.lng]), this.group = this.svg.append("g"), this.road_points = [[this.bp_px.x, this.bp_px.y], [this.mp_px.x, this.mp_px.y], [this.ep_px.x, this.ep_px.y]], this.clipPath = this.group.append("defs").append("clipPath").attr("id", "aaa").attr("clip-rule", "evenodd");
        ...
        var a = this.w_plane, b = this.h_plane, c = this.pos_plane.x, d = this.pos_plane.y, e = this.spos + .01, f = this.road.node().getTotalLength(), g = this.road.node().getPointAtLength(e * f), h = Victor(g.x - c.x, g.y - c.y).angleDeg();
        this.rot = h + 45;
        this.spos;
