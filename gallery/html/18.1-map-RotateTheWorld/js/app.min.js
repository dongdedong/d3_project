!function () {
    function t(t, n, e) {
        var a = t.translate(), o = Math.atan2(n[1] - a[1], n[0] - a[0]) - Math.atan2(e[1] - a[1], e[0] - a[0]);
    ...
    function e(t) {
        var n = .5 * t[0] * d, e = .5 * t[1] * d, a = .5 * t[2] * d, o = Math.sin(n), r = Math.cos(n), c = Math.sin(e), i = Math.cos(e), s = Math.sin(a), l = Math.cos(a);
        return [r * i * l + o * c * s, o * i * l - r * c * s, r * c * l + o * i * s, r * i * s - o * c * l]
    }

    function a(t, n) {
        var e = t[0], a = t[1], o = t[2], r = t[3], c = n[0], i = n[1], s = n[2], l = n[3];
        return [e * c - a * i - o * s - r * l, e * i + a * c + o * l - r * s, e * s - a * l + o * c + r * i, e * l + a * s - o * i + r * c]
    }

    function o(t, n) {
        if (t && n) {
            var e = l(t, n), a = Math.sqrt(s(e, e)), o = .5 * Math.acos(Math.max(-1, Math.min(1, s(t, n)))), r = Math.sin(o) / a;
            return a && [Math.cos(o), e[2] * r, -e[1] * r, e[0] * r]
        }
    }

    function r(t, n) {
        var e = Math.max(-1, Math.min(1, s(t, n))), a = 0 > e ? -1 : 1, o = Math.acos(a * e), r = Math.sin(o);
        return r ? function (e) {
            var c = a * Math.sin((1 - e) * o) / r, i = Math.sin(e * o) / r;
    ...
    function i(t) {
        var n = t[0] * d, e = t[1] * d, a = Math.cos(e);
    ...
    var d = Math.PI / 180, h = 180 / Math.PI;
    d3.geo.zoom = function () {
        ...
        var h, f, p, v = 0, g = u(m, "zoomstart", "zoom", "zoomend"), m = d3.behavior.zoom().on("zoomstart", function () {
            var r = d3.mouse(this), i = e(h.rotate()), u = n(h, r);
            u && (p = u), M.call(m, "zoom", function () {
                h.scale(z.k = d3.event.scale);
                var e = d3.mouse(this), s = o(p, n(h, e));
        ...
        return m.rotateTo = function (t) {
            ...
            t.each(function () {
                var t = d3.select(this), n = g.of(this, arguments), a = z, o = d3.transition(t);
                if (o !== t) {
                    o.each("start.zoom", function () {
                        this.__chart__ && (z = this.__chart__), h.rotate(z.r).scale(z.k), s(n)
                    }).tween("zoom:zoom", function () {
                        var t = m.size()[0], i = r(e(z.r), e(a.r)), s = d3.geo.distance(z.r, a.r), u = d3.interpolateZoom([0, 0, t / z.k], [s, 0, t / a.k]);
    ...
    function t(t, n, e) {
        var a = n.projection();
        t.append("path").datum(d3.geo.graticule()).attr("class", "graticule").attr("d", n), t.append("path").datum({type: "Sphere"}).attr("class", "foreground").attr("d", n).on("mousedown.grab", function () {
            var n;
            e && (n = t.insert("path", ".foreground").datum({
                type: "Point",
                coordinates: a.invert(d3.mouse(this))
            }).attr("class", "point").attr("d", o));
            var o = d3.select(this).classed("zooming", !0), r = d3.select(window).on("mouseup.grab", function () {
    ...
    var e = 180 / Math.PI, a = 1e3, o = 600, r = d3.dispatch("world"), c = -1;
    d3.selectAll("#map, #eisenlohr").data([n(a, o).scale(245).translate([a / 2, .56 * o]), d3.geo.eisenlohr().precision(.1).clipExtent([[-1, -1], [a + 1, o + 1]]).translate([a / 2, o / 2]).scale(75).rotate([0, -30])]).append("svg").attr("width", a).attr("height", o).each(function (n) {
        var e = d3.geo.path().projection(n), a = d3.select(this).call(t, e, !0);
        ...
        var a = 350, o = 350, c = n(a, o), i = n(a, o), s = d3.geo.path().projection(c), l = d3.geo.path().projection(i), u = d3.select("#comparison").append("svg").attr("width", a).attr("height", o).call(t, s, !0);
        ...
        function a(e, a, u) {
            ...
            var h = n(i, s).rotate([0, 0]), f = d3.geo.path().projection(h);
            o.push(h);
            var p = o.slice(), v = d3.select("#rotations").insert("svg", "*").datum({path: f}).attr("width", i).attr("height", s + 20).call(t, f), g = v.append("text").attr("x", i / 2).attr("y", s + 10).attr("text-anchor", "middle").text(e + "=0\xb0");
            r.on("world." + ++c, function () {
                v.selectAll("path").attr("d", f)
            });
            var m = d3.behavior.drag().on("drag", function () {
                for (var t = 0; t < p.length; ++t) {
                    var n = p[t], a = d(n.rotate());
                    n.rotate(a.rotate)
                }
        ...
        var o = [], i = 220, s = 220, l = d3.format(".1f");
        a("\u03b3", function (t) {
            var n = d3.event.x - i / 2, a = s / 2 - d3.event.y;
    ...
    }(), d3.json("data/world-110m.json", function (t, n) {
